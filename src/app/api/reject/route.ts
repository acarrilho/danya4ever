...
export async function GET(req: NextRequest) {
  ...
  if (!id || !token) {
    return html(errorPage('Invalid rejection link.'), 400)
  }
  ...
  if (message.status === 'rejected') {
    return html(successPage('This message was already rejected.'), 200)
  }
  ...
  if (updateError) {
    return html(errorPage('Failed to reject message. Please try again.'), 500)
  }

  return html(successPage('Message rejected and will not appear publicly.'), 200)
}
...
function successPage(msg: string): string {
  ...
  return page('✓ Done', msg, '#92400e', '#fffbeb',
