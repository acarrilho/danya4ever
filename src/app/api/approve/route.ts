...
export async function GET(req: NextRequest) {
  ...
  if (!id || !token) {
    return new NextResponse(errorPage('Invalid approval link.'), {
      status: 400,
      headers: { 'Content-Type': 'text/html' },
    })
  }

  // Verify token matches and message is still pending
  ...
  if (message.status === 'approved') {
    return new NextResponse(successPage('Message was already approved.'), {
      status: 200,
      headers: { 'Content-Type': 'text/html' },
    })
  }
  ...
  if (updateError) {
    return new NextResponse(errorPage('Failed to approve message. Please try again.'), {
      status: 500,
      headers: { 'Content-Type': 'text/html' },
    })
  }

  return new NextResponse(successPage('Message approved and now visible on the memorial board.'), {
    status: 200,
    headers: { 'Content-Type': 'text/html' },
  })
}
...
function successPage(msg: string): string {
  ...
  return page('✓ Approved', msg, '#166534', '#f0fdf4', `<a href="${baseUrl}/admin" style="color:#b89a5c;">Go to admin dashboard</a>`)
}
