...
import { isAdminAuthenticated, SESSION_COOKIE } from '@/lib/admin-auth'
...
export default async function AdminPage() {
  ...
  const { messages, error } = await getAllMessages()
  ...
  return (
    <div className="min-h-screen bg-[#faf7f0]">
      ...
      <header className="bg-white border-b border-[#e7e0d4] sticky top-0 z-10">
        <div className="max-w-4xl mx-auto px-4 py-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-stone-400">In Memory of Daniel Naroditsky</p>
          </div>
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        {/* Pending section — most prominent */}
        <section className="mb-8">
          ...
          {pending.length === 0 ? (
            ...
            <div className="space-y-3">
              {pending.map((msg) => (
                <AdminMessageRow key={msg.id} message={msg} />
              ))}
            </div>
          )}
        </section>
